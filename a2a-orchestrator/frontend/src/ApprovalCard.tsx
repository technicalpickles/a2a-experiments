...
function ArgsBlock({ permission }: { permission: Permission }) {
  // `permission` is the wire payload cast straight from the tool args — treat
  // every field as optional. A malformed one must render empty, not throw:
  // there is no error boundary above this, so a throw here unmounts the app.
  const input = permission.input ?? {}
  const { command, description, ...rest } = input as {
    command?: string
    description?: string
    [k: string]: unknown
  }
  ...
  return (
    <pre className="m-0 overflow-x-auto whitespace-pre-wrap rounded-sm border border-dashed border-[oklch(0.80_0.03_150)] px-[13px] py-[11px] font-mono text-[12.5px] leading-[1.75] max-md:break-all dark:border-[oklch(0.32_0.04_150)]">
      {typeof command === 'string' ? command + '\n' : null}
      {desc ? (
        <span className="text-[oklch(0.45_0.04_150)] dark:text-[oklch(0.60_0.03_150)]">
          {'# ' + desc + '\n'}
        </span>
      ) : null}
      {restKeys.length > 0 || typeof command !== 'string'
        ? JSON.stringify(typeof command === 'string' ? rest : input, null, 2)
        : null}
    </pre>
  )
}
...
export function ApprovalCard({
    ...
    return (
      <div
        className={
          'phosphor-approval ' +
          (denied
            ? 'rounded-sm border border-[oklch(0.78_0.10_340)] bg-[oklch(0.94_0.03_340)] opacity-85 dark:border-[oklch(0.45_0.16_340)] dark:bg-[#120a10]'
            : 'rounded-sm border border-border bg-card opacity-72 dark:border-[oklch(0.32_0.04_150)] dark:bg-[#0b0d0c]')
        }
      >
        <div className="flex items-center justify-between px-4 pt-2.5 text-[11px] tracking-[.14em]">
          <span
            className={
              denied
                ? 'text-[oklch(0.45_0.20_340)] dark:text-[oklch(0.82_0.14_340)]'
                : 'text-[oklch(0.42_0.03_150)] dark:text-[oklch(0.70_0.02_150)]'
            }
          >
        ...
        <div className="px-4 pb-3 pt-1 text-[12.5px]">
          ...
          {denied ? (
            <div className="text-[oklch(0.45_0.20_340)] dark:text-[oklch(0.82_0.14_340)]">
              agent told: user denied this tool call
            </div>
          ) : null}
        </div>
      </div>
  ...
  return (
    <div
      className="phosphor-approval rounded-sm border border-primary bg-[oklch(0.96_0.025_145)] max-md:-mx-5 dark:bg-[#0b120d]"
      ref={(el) => {
